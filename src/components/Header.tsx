...
const Header = () => {
  ...
  useEffect(() => {
    console.log(`link is ${window.location.origin}/linkedin`);
  }, []);

  // const { linkedInLogin } = useLinkedIn({
  //   clientId: "863mm46hr7of6u",
  ...
  //       console.log("User Info linkedin :", userInfo);
  //     }
  //   },
  //   onError: (error) => {
  //     console.log(error);
        ...
        {context?.user ? (
          ...
          <div
            ...
            onClick={() => {
                googleLogin();
            //   linkedInLogin();
              //   const linkedinRedirectUrl = `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=863mm46hr7of6u&redirect_uri=http://localhost:5173/linkedin&scope=profile,email`;
...
export const fetchLinkedinUserInfo = async (accessToken: any) => {
  localStorage.setItem("access_token", accessToken);
